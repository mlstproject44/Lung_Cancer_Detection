...
def create_ellipsoid_mask(  #create a binary mask with a filled ellipsoid accounting for anisotropic voxel spacing
    ...
    y_max = min(shape[1], center_y + max_radius + 1)  # FIXED: was shape[0]
    x_min = max(0, center_x - max_radius)
    x_max = min(shape[2], center_x + max_radius + 1)  # FIXED: was shape[0]
    ...
    zz, yy, xx = np.meshgrid(
        np.arange(z_min, z_max),  # FIXED: was np.arrange
        np.arange(y_min, y_max),  # FIXED: was np.arrange
        np.arange(x_min, x_max),  # FIXED: was np.arrange
        indexing='ij'
    )
...
if __name__ == "__main__":
    base_dir = r"C:\Users\emirb\OneDrive\Desktop\coding\Python\DSAI\projects\data\luna16_downsampled"
    annotations_file = r"C:\Users\emirb\OneDrive\Desktop\coding\Python\DSAI\projects\data\data_preprocessing\annotations.csv"
    output_dir = r"C:\Users\emirb\OneDrive\Desktop\coding\Python\DSAI\projects\data\luna16_masks"

    print("Creating segmentation masks for all LUNA16 subsets...")

    create_masks_for_all_subsets(base_dir, annotations_file, output_dir, num_subsets=10)
    print("\nDone!")
